...
from lib.routes.util import URLGenerator
...
class WSGIApplication(object):
  ...
  def __call__(self, environ, start_response):
    ...
    result = self.mapper.routematch(environ = environ)
    if result:
      match, route = result
      handler = None
      groups = dict([(x,match[x]) for x in match if x not in ['controller', 'action']])
      handler_cls = ClassImport(self.handlerMap[match['controller']])\
                    .new_factory(op=match['action'])
      handler = handler_cls()
      handler.initialize(request, response)
      
      try:
        handler.route(**groups)
      except Exception, e:
