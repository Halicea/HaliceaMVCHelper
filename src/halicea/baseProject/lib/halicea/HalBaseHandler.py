...
from lib.halicea.helpers import DynamicParameters, LazyDict, ContentTypes, NotSuportedException
...
class HalBaseHandler(object):  
  def __init__(self, *args, **kwargs):
    ...
    self.__template__ =""
    self.extra_context ={}
    self.dispatcher = ClassImport(settings.ACTION_DISPATCHER)()
...
#   the method by the operation
  ...
  def get_response_dict(self, basedict ):
    ...
    #forms
    result.update(paths.GetFormsDict(os.path.join(settings.FORM_VIEWS_DIR, self.TemplateType))) ##end
  ...
  def get_url(cls, *args, **kwargs):
    return WSGIApplication.app.url(*args, **kwargs)
