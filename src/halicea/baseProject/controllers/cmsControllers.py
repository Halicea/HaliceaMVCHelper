...
class CMSLinksController(CMSBaseController):
  ...
  def save(self, *args):
    ...
    isUnique =not cms.CMSLink.all()\
                      .filter("ParentLink =",parent)\
                      .filter("AddressName =",addressName)\
                      .get()
                      
    if not isUnique:
      errors.append('Error: There is Already a link with the same address')
...
class MenuController(CMSBaseController):
  ...
  @ResponseType(lambda r:r.isAjax and 'json' or 'html')
  def index(self,*args):
...
class CMSPageController(CMSBaseController):
  ...
  def view(self, url):
    ...
    if lnk:
      ...
      self.redirect(self.get_url(controller='login', action='login'))
