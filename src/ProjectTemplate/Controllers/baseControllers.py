...
class LoginController( MyRequestController ):
	@ErrorSafe()
	def get( self, *args ):
		if not self.User:
...
class LogoutController( MyRequestController ):
	@LogInRequired(message = '')
	def get( self, *args ):
		self.logout_user()
		self.redirect( LoginController.get_url() )

class AddUserController( MyRequestController ):
	def get( self ):
		self.respond()
		
	def post( self ):
		self.SetTemplate(ControllerType, 'Thanks.html')
		try:
			user = base.Person( 
...
class WishListController(MyRequestController):
	def get(self):
